...
public class Hexabus implements Closeable {
    ...
    private final Queue<HexabusPacket> receivedPackets;
    ...
    public Hexabus(DatagramSocket socket) {
        this.socket = socket;
        receivedPackets = new ConcurrentLinkedQueue<HexabusPacket>();
    }
    ...
    public HexabusPacket receivePacket(InetAddress from) throws IOException {
        if (from != null) {
            synchronized (this) {
                for (HexabusPacket p : receivedPackets) {
                    if (from.equals(p.getSourceAddress())) {
                        receivedPackets.remove(p);
                        return p;
                    }
                }
            }
        ...
        while (true) {
            try {
                ...
                if (from == null || from.equals(p.getSourceAddress())) {
                    return p;
                } else {
